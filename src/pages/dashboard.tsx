...
import { getSalesAssociates } from 'apiSdk/sales-associates';
...
function DashboardPage() {
  ...
  const salesAssociateStat = useSWR<PaginatedInterface<SalesAssociateInterface>>(
    () => `/sales-associates`,
    () => getSalesAssociates({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: salesAssociateStat?.data?.totalCount,
      isLoading: salesAssociateStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Tools',
